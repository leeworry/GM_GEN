from fewshot_re_kit.data_loader import get_loader, get_loader_pair, get_loader_unsupervised, get_loader_pair2, get_loader2, collate_fn_val
...
from models.gm_gen import Gm_Gen
...
def main():
    ...
    parser.add_argument('--val', default='val_wiki',
            help='val file')
    parser.add_argument('--test', default='test_wiki_input-5-5',
            help='test file')
    ...
    parser.add_argument('--model', default='gm_gen',
            help='model name')
    ...
    parser.add_argument('--param_lr', type=float, default=0.0001,
                        help='checkpoint name.')
    parser.add_argument('--test_lr', type=float, default=0.,
                        help='checkpoint name.')
    ...
    if opt.pair:
        ...
        val_params_data_loader = get_loader_pair(opt.val, sentence_encoder,
        ...
        val_params_data_loader = get_loader(opt.val, sentence_encoder,
                                            N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=1,
    ...
    if opt.adv:
        ...
        # framework = FewShotREFramework(train_data_loader, val_params_data_loader, test_data_loader, adv_data_loader, adv=opt.adv, d=d)
    elif opt.test_params:
        framework = FewShotREFramework(train_data_loader, val_params_data_loader, test_data_loader)  # params
    else:
    ...
    elif model_name == 'gm_gen':
        model = Gm_Gen(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder, dropout=opt.dropout)
    elif model_name == 'gnn':
    ...
    if not opt.only_test and (not opt.test_online):
        ...
        print('this is the test generation type.')
        ckpt = opt.load_ckpt
        if ckpt is None:
            print("Warning: --load_ckpt is not specified. Will load Hugginface pre-trained checkpoint.")
            ckpt = 'none'
        framework.test_param(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair,
                             test_output=opt.test_output, test_lr=opt.param_lr)  # params
        ...
        if ckpt is None:
            print("Warning: --load_ckpt is not specified. Will load Hugginface pre-trained checkpoint.")
            ckpt = 'none'
        framework.test(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, test_output=opt.test_output, test_lr=opt.param_lr)
